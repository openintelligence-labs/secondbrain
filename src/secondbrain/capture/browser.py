    ...
    *,
    ...
    try:
        ...
        if since is not None:
            chromium_epoch = (since - datetime(1601, 1, 1, tzinfo=timezone.utc)).total_seconds() * 1_000_000
        ...
        for url, title, last_visit in rows:
            ts = datetime(1601, 1, 1, tzinfo=timezone.utc) + timedelta(microseconds=int(last_visit))
            yield Capture(
